...
def create_payment_fito(dannye_payment: dict) -> None:
    ...
    ws['B1'] = 'Valstybinės fitosanitarijos tarnybos naudai avanso sumokėjimą patvirtinantis dokumentas'

    ws.merge_cells('B2:M2')
    ws['B2'].alignment = Alignment(horizontal='center')
    ws['B2'] = 'Kontroliuojamų prekių partijos valstybinė fitosanitarinė kontrolė, pavyzdinė forma'
    ...
    ws['B6'] = 'Aš, žemiau pasirašęs MB Alesta LT įgaliotas asmuo, patvirtinu, kad už žemiau  nurodytą valstybinei fitosanitarijos'
    ws['B6'].font = Font(size=8)

    ws.merge_cells('B7:M7')
    ws['B7'].alignment = Alignment(horizontal='center')
    ws['B7'] = 'kontrolei pateikiamą siuntą yra iš anksto sumokėta valstybės rinkliava:'
    ...
    top_left_cell = ws['K9']
    top_left_cell.value = dannye_payment['summ_to_pay']
    top_left_cell.font = Font(underline='single')
    ...
    top_left_cell.value = f"{dannye_payment['numb_truck']}, {dannye_payment['driver']}"
    top_left_cell.font = Font(underline='single')
    ...
    top_left_cell.value = '3. Siuntą lydinčio fitosanitarijos sertifikato ar kito fitosanitarijos ir kokybę patvirtinančio dokumento numeris ir data: '
    top_left_cell.font = Font(size=8)
    ...
    #top_left_cell.value = f"{dannye_payment['svid']}, {dannye_payment['dte_svid']}"
    #
    ...
    top_left_cell.value = '4. Pasienio fitosanitarijos posto, pro kurį įvežama siunta  į Lietuvos Respublikos teritoriją, pavadinimas'
    top_left_cell.font = Font(size=8)
    ...
    top_left_cell.value = f"{dannye_payment['chedpp']} {dannye_payment['dte_chedpp']}"

    top_left_cell.alignment = Alignment(horizontal='center', wrapText=True)
    top_left_cell.font = Font(underline='single')
    top_left_cell.border = Border(left=thins, right=thins)
    if len(f"{dannye_payment['chedpp']} + ' ' + {dannye_payment['dte_chedpp']}") > 90:
        ws.row_dimensions[23].height = ceil(
            len(f"{dannye_payment['chedpp']} {dannye_payment['dte_chedpp']}") / 100) * 14.4
    ...
    top_left_cell.font = Font(size=8)
    top_left_cell.border = Border(left=thins, right=thins)
    top_left_cell.value = '8. Pasienio fitosanitarijos  posto  pareigūnas: '
    ...
    folder_path = ensure_valid_path("platezhka_fito")

    # Название файла (без расширения)
    base_name = dannye_payment['platezhka']
